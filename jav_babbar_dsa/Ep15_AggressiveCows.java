...
public class Ep15_AggressiveCows {
    public static boolean isCowsPlaced(int[] positions, int dist, int cows) {
        int lastPos = positions[0];
        cows--;
        for (int i = 1; i < positions.length; i++) {
            if (positions[i] - lastPos >= dist) { //means, we can place cow at positions[i]
                lastPos = positions[i];
                cows--;
                if (cows == 0)
                    return true;
    ...
    public static int aggressiveCows(int[] nums, int k) {
        ...
        while (l <= r) {
            ...
            if (isCowsPlaced(nums, mid, k)) {
                ans = mid;
                l = mid + 1; //we're going right side, since we need largest dist
            } else {
                r = mid - 1;
            }
        }
